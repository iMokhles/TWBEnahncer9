...
- (void)viewDidLoad
{
    ...
    _consumerKeyTextField.text = @"j9s7DQQWZJR77gGACLxpzVHOf";
    _consumerSecretTextField.text = @"MjdTGSGrunMCBBouQmg737FZ3ciFGYV7nJ0VrhfYNX1bDBg1t6";
}
...
- (IBAction)getTimelineAction:(id)sender {
    ...
    [[PHImageManager defaultManager] requestImageDataForAsset:lastAsset options:PHImageRequestOptionsVersionCurrent resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
        if (imageData) {
            [_twitter postMediaUploadData:imageData fileName:@"image.png"
                     uploadProgressBlock:^(int64_t bytesWritten, int64_t totalBytesWritten, int64_t totalBytesExpectedToWrite) {
                         ...
                         NSLog(@"******* image MediaID: %@", mediaID);
                         NSLog(@"******* image info: %@", imageDictionary);
                         [_twitter postDirectMessage:@"" forScreenName:@"" orUserID:@"" successBlock:^(NSDictionary *message) {
                             //
